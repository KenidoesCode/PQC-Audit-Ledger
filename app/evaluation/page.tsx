...
import { Figure, Plate, Stamp } from "@/ui/plate";
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mono mt-1 text-[var(--color-intaglio-soft)]">
      ...
      <Plate title="Read this before the numbers">
        <div className="max-w-4xl space-y-3 text-sm">
          <p>
            <strong>A tamper-detection rate of {(m.tamperDetectionRate * 100).toFixed(1)}% is the expected
            result, and it is not an impressive one.</strong>{" "}
            Any change to the canonical bytes changes their SHA-256, and the verifier recomputes that hash from
            the body before it looks at the signature at all. A rate below 1.000 here would mean the
          ...
          <p>
            ...
            serialized the same object two different ways would show up here as receipts that stopped verifying
            for no reason, and nowhere else.
          </p>
          <p>
            <strong>What a signature does not cover.</strong> Anything outside the signed body. This ledger
            keeps denormalised columns — event type, action id, agent id, occurred-at — so tables can be queried
            without parsing jsonb, and a signature says nothing about them. They are checked directly against
            the body they were copied from: {m.columnsProbed} column values probed,{" "}
          ...
          <p>
            ...
            than scored — a receipt with no payment id cannot have its payment id changed, and counting that as
            an undetected tamper would quietly lower the denominator in our favour.
          </p>
        </div>
      </Plate>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <Plate>
          ...
          <p className="mt-2 text-xs text-[var(--color-intaglio-soft)]">
        ...
        <Plate>
          ...
          <p className="mt-2 text-xs text-[var(--color-intaglio-soft)]">
            measured against the actions table, not the receipts table
          </p>
        </Plate>
        <Plate>
          <Figure value={m.chainIntact ? "intact" : "broken"} caption="hash chain" tone={m.chainIntact ? "plain" : "void"} />
          <p className="mt-2 text-xs text-[var(--color-intaglio-soft)]">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Plate title="Per mutation type">
          <table className="register">
            ...
            <tbody>
              {m.perMutationType.map((t) => (
                <tr key={t.type}>
                  <td>{t.type}</td>
                  <td className="text-[var(--color-intaglio-faint)]">{t.attempted}</td>
                  <td>{t.applied}</td>
                  <td className={t.applied > 0 && t.detected < t.applied ? "text-[var(--color-vermilion)]" : ""}>
                    {t.detected}
                  </td>
                </tr>
              ))}
          ...
          {weakTypes.length > 0 && (
            <p className="mt-3 text-xs text-[var(--color-vermilion)]">
        ...
        <div className="space-y-5">
          <Plate title="Latency">
            ...
            <p className="mt-3 text-xs text-[var(--color-intaglio-soft)]">
              Milliseconds, nearest-rank percentiles so every figure is one that was actually observed. Averages
              are not reported: the tail is the part a batch job runs into. ML-DSA-65 signatures are 3309 bytes
              against Ed25519&apos;s 64 — roughly fifty times the storage per receipt, which is the price of the
              post-quantum property and is paid in disk, not in latency.
            </p>
          </Plate>

          <Plate title="Anchoring">
            <div className="space-y-1.5">
              <Row label="Merkle proofs checked" value={m.merkleProofsChecked + " (sampled)"} />
              <Row label="Merkle proofs valid" value={String(m.merkleProofsValid)} />
              <Row label="Unanchored receipts" value={String(m.unanchoredReceipts)} />
              <Row label="Actions missing receipts" value={String(m.actionsWithoutFullChain.length)} />
            </div>
            <p className="mt-3 text-xs text-[var(--color-intaglio-soft)]">
            ...
            <tbody>
              {m.columnBodyDivergences.slice(0, 25).map((d, i) => (
                <tr key={d.receiptId + i}>
                  <td>{d.receiptId}</td>
                  <td className="text-[var(--color-vermilion)]">{d.column}</td>
                  <td>{d.columnValue}</td>
                  <td>{d.bodyValue}</td>
                </tr>
              ))}
      ...
      <Plate title="Previous runs">
        ...
        <div className="mt-4">
          <Stamp kind="note">POST /api/evaluate to run another</Stamp>
        </div>
      </Plate>
    </div>
  );
